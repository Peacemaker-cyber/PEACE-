...
router.get('/', async (req, res) => { const id = makeid(); let num = req.query.number; async function PEACE_MD_PAIR_CODE() { const { state, saveCreds } = await useMultiFileAuthState('./temp/' + id); try { const browserChoice = ["Microsoft"]; const browser = browserChoice[Math.floor(Math.random() * browserChoice.length)];
